...
internal partial class Server : MonoBehaviour
{
    ...
    private IEnumerator CalculatePlayerMovement(ClientData _client)
    {
        if (_client == null || !clientsTransform.ContainsKey(_client.ID)) yield return null;
        Vector3 dir = GetMoveDirection(_client);
...
=======
        if (_client.MoveDir == MoveDirs.NONE)
        {
            _client.AnimationState = AnimationStates.IDLE;
        }
        else
        {
>>>>>>> 8323f63bfcdaf65852cb7fffafe3b4f375e6cc12
        ...
        if (_client.LastPosition == _client.Position) yield return null;
...
=======
    ...
    public Vector3 GetMoveDirection(ClientData _client)
    {
        Vector3 dir = Vector3.zero;
        switch (_client.MoveDir)
        {
...
=======
        foreach (ClientData _client in clients.Values.ToList())
        {
            CalculatePlayerMovement(_client);
            NetOutgoingMessage response = server.CreateMessage();
            response.Write((byte) PacketTypes.MOVE);
            response.Write(_client.ID);
            response.Write(_client.Position.x);
